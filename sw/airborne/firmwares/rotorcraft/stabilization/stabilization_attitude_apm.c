...
#define D_T (1.0/PERIODIC_FREQUENCY)
...
struct FloatRates rate_err_last;
...
void stabilization_attitude_init(void)
{
  ...
  VECT3_ASSIGN(stabilization_gains.rates_p,
               STABILIZATION_ATTITUDE_PHI_RATE_PGAIN,
               STABILIZATION_ATTITUDE_THETA_RATE_PGAIN,
               STABILIZATION_ATTITUDE_PSI_RATE_PGAIN);
...
void stabilization_attitude_enter(void)
{
  ...
  FLOAT_EULERS_ZERO(stabilization_att_sum_err);
  FLOAT_RATES_ZERO(rate_err_last);
}
...
void stabilization_attitude_run(bool  in_flight)
{
 ...
 // EULERS_BOUND_CUBE(att_err,-0.1, 0.1);
  // if (in_flight) 
    /* update integrator */
  ...
  /*  rate error                */
  struct FloatRates rates_bef_matrix;
  struct FloatRates rates_aft_matrix;
  ...
  */
    ...
    rates_bef_matrix.p = stabilization_gains.rates_p.x * att_err.phi; //phi dot
    rates_bef_matrix.q = stabilization_gains.rates_p.y * att_err.theta; // theta dot
    rates_bef_matrix.r = stabilization_gains.rates_p.z * att_err.psi; // psi dot

    //multiply matrix

    rates_aft_matrix.p = rates_bef_matrix.p - (rates_bef_matrix.r * sinf(att_float->theta)) ;
    rates_aft_matrix.q = (rates_bef_matrix.q * cosf(att_float->phi)) + (sinf(att_float->phi) * cosf(att_float->theta) * rates_bef_matrix.r) ;
    rates_aft_matrix.r = ((-1) * sinf(att_float->phi) * rates_bef_matrix.q) + (cosf(att_float->phi) * cosf(att_float->theta) * rates_bef_matrix.r) ;

    RATES_DIFF(rate_err, rates_aft_matrix , *rate_float);

    if (in_flight) {
    /* update integrator */
    stabilization_att_sum_err.phi += rate_err.p;
    stabilization_att_sum_err.theta += rate_err.q;
    stabilization_att_sum_err.psi += rate_err.r;
    ...
    stabilization_att_fb_cmd[COMMAND_ROLL] = 
    stabilization_gains.p.x * rate_err.p + 
    stabilization_gains.i.x * stabilization_att_sum_err.phi +
    stabilization_gains.d.x * (rate_err.p - rate_err_last.p) / D_T;
    
    stabilization_att_fb_cmd[COMMAND_PITCH] = 
    stabilization_gains.p.y * rate_err.q +
    stabilization_gains.i.y * stabilization_att_sum_err.theta +
    stabilization_gains.d.y * (rate_err.q - rate_err_last.q) / D_T;

    stabilization_att_fb_cmd[COMMAND_YAW] = 
    stabilization_gains.p.z * rate_err.r +
    stabilization_gains.i.z * stabilization_att_sum_err.psi +
    stabilization_gains.d.z * (rate_err.r - rate_err_last.r) / D_T;

    RATES_COPY(rate_err_last, rate_err);
